...
// --- Style dla fioletowych przycisków ikon z większym zaokrągleniem ---
const iconButtonBaseStyle = "p-2 rounded-2xl transition-all duration-150 ease-in-out shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2"; // Bardziej zaokrąglony
const primaryIconButton = `${iconButtonBaseStyle} bg-[#c5baff] text-white hover:bg-[#b1a5f0] focus:ring-[#c5baff] active:bg-[#a89dd9] active:shadow-inner`; // Fioletowy
// ------------------------------------------------------------------

export function Header() {
  const [isDarkMode, setIsDarkMode] = useState(false); 

  const toggleTheme = () => { setIsDarkMode(!isDarkMode); console.log("TODO: Toggle theme"); };
  const toggleLanguage = () => { console.log("TODO: Toggle language"); };

  return (
    // Białe tło nagłówka z subtelną linią dolną
    <header className="sticky top-0 z-50 bg-white border-b border-gray-200">
      <nav className="container mx-auto px-4 sm:px-6 py-3 flex items-center gap-6 sm:gap-8">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3 ml-auto">
          {/* Przycisk Motywu - fioletowy, bardziej zaokrąglony */}
          <button
            onClick={toggleTheme}
          ...
          {/* Przycisk Języka - fioletowy, bardziej zaokrąglony */}
           <button
          ...
          {/* Przycisk Portfela (już ma fioletowy styl i nowe zaokrąglenie) */}
          <ConnectButton />
        </div>
      </nav>
    </header>
